...
from cocoex.write_fun import write_points_to_file
...
def batch_loop(solver, suite, observer, budget,
    ...
    for problem_index, problem in enumerate(suite):
        if problem_index % 15 == 0:  # runs 15 times, only once is needed
            problem_num = problem.id[7:9]  # create a string of the number of the function
...
#===============================================
def coco_optimize(solver, fun, max_evals, max_runs=1e9, counter =1):
...
############################ ADD HERE ########################################
        ...
        # elif solver.__name__ == 'write_points_to_file':
        if solver.__name__ == 'write_points_to_file':
            solver(fun, fun.lower_bounds, fun.upper_bounds, counter)
##############################################################################
        """
        else:
            solver(fun, x0)
...
if __name__ == '__main__':
    ...
    for i in range(10):  # num of times i extract points
        if i % 100 == 0:
            print(str(i)+"\n\n\n\n\n\n")
        main(budget, max_runs, current_batch, number_of_batches)
        os.removedirs(r'C:\Users\ido\PycharmProjects\python\exdata\default')
